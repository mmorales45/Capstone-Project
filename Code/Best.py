'''
To run this function that generates the "Best" result, run the following line.
...
def Full_program():
    ...
    #Create csv file
    np.savetxt("Best_config.csv", next_config, delimiter= ",")
    np.savetxt("Best_Xerr.csv", X_err_list, delimiter= ",")
    y1 = []
    y2 = []
    y3 = []
    y4 = []
    ...
    plt.title(f"Best, Kp value:{kp_multiplier},Ki value:{ki_multiplier}")  
    plt.legend()
    plt.show()
    

if __name__ == "__main__": # the main function
    logging.info('python3 Best.py')
    logging.info('Starting the Main Program for Best!')
    Full_program()
